...
public class InOutCallTemplateTestCase {
	private final static Logger log = LoggerFactory.getLogger(InOutCallTemplateTestCase.class);
	...
	public void testCall_ExplicitTransaction() {
		...
		expect(tOps.execute((TransactionCallback<Object>) anyObject()))
				.andReturn(ret);
	...
	public void testCallForObjectWithClass() {
		...
		replay(jdbcOperations);
		try { 
			object = call.callForObject("test", Integer.class);
	...
	public void testCallForObjectWithRowMapper() {
		...
		replay(jdbcOperations);
		try { 
			object = call.callForObject("test", ReusableRowMappers.ROW_MAPPER_INTEGER);
