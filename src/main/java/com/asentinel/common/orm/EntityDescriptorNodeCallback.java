...
public interface EntityDescriptorNodeCallback {
	 ...
	 * 			implementation cannot provide an implementation. The {@link DefaultEntityDescriptorTreeRepository} can be configured with
	 ...
	 */
	default boolean customize(Node<EntityDescriptor> node, SimpleEntityDescriptor.Builder builder,
	 ...
	 */
	boolean customize(Node<EntityDescriptor> node, SimpleEntityDescriptor.Builder builder);
	 ...
	 * 			root table.  
	 */
	static EntityDescriptorNodeCallback rootOnlyQuery() {
...
class EntityDescriptorNodeCallbackConstants {
    ...
    static final EntityDescriptorNodeCallback NO_QUERY_BELOW_ROOT  = (n, b) -> {
		if (!n.isRoot()) {
